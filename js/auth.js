...
export async function sendMagicLinkViaREST(email,redirectTo){try{const response=await fetch(SUPABASE_URL+'/auth/v1/otp',{method:'POST',body:JSON.stringify({email:email})});if(!response.ok){return{error:'發送失敗'}}return{success:true}}catch(err){return{error:err.message}}}
